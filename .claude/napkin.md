...
1. **[2026-09-21] Phase 1: install systemd units for relay + tunnel (place ngrok permanently, e.g. `/usr/local/bin`), reboot-test for a stable hostname**
   Do instead: confirm the tunnel survives a reboot with the *same* hostname before touching the Alexa console at all — a rotating hostname was the prior prototype's documented failure mode. **Needs netbook SSH/LAN access — not implementable from a remote/cloud session.**
...
2. **[2026-09-21] Phase 2: switch the reused skill's endpoint from Lambda ARN to HTTPS, edit interaction model off `TalkIntent` toward ADR 0005's shape**
   Do instead: expect several "Save Model" failures (ADR 0005); keep the two-turn fallback (LaunchRequest asks the question, then captures the follow-up) ready if the single-shot slot won't validate. Endpoint switch is per ADR 0003/0009 — same Skill ID, new endpoint config. **The actual cutover needs interactive Amazon Developer Console access — not implementable from a remote/cloud session** (repo-side prep, e.g. drafting `interaction-model.json` to paste in, would be).

3. **[2026-09-21] Phase 4 option: local llama.cpp backend is faster than OpenRouter but blocked by a reasoning-mode truncation bug — a plain server restart does NOT fix it**
   Do instead: measured (ADR 0012) — local (192.168.4.55:11434, `LFM2.5-2.6B-Q4_K_M.gguf`) beats OpenRouter 3-5x on latency (p90 ~1.3s vs 6.08s) with no rate-limit exposure, but reasoning eats the shared `max_tokens` budget and no per-request flag disables it. Already tried and ruled out: restarting the server without whatever flag enabled the `reasoning_content` split -- this just inlines the raw `<think>...</think>` block into `content` instead (worse: would be read aloud verbatim), still truncates. Needs an actual `--reasoning-format none`-style launch flag before re-testing via `measure_latency.py --base-url ...`. `app.py`'s `_call_chat_completions` already supports this with zero new code once the server's fixed. **Needs LAN access to the Windows PC (192.168.4.55) — not implementable from a remote/cloud session.**
...
7. **[2026-09-22] apt's `cryptography` 2.1.4 (ADR 0007) predates path-building APIs — root-of-trust is now built by hand against certifi instead**
   Do instead: `relay/app.py`'s `_verify_chain_root_of_trust` (ADR 0013) indexes certifi's CA bundle by subject and verifies the topmost fetched cert against it, closing the gap without a `cryptography` upgrade. Written and reasoned through in a remote session with no live Alexa request to test against — **still needs a real-chain test on the netbook before Phase 2's live cutover**; don't assume it's field-verified just because it's merged.
